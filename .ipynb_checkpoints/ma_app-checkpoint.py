...
#     data = api.get_barset(ticker, 'day', limit=days)[ticker].df.reset_index()
    data = api.get_barset(ticker, 'day', limit=days)[ticker].df
...
def plot(data,windows,strategy):
    fig, ax = plt.subplots()
    ax.plot(data['close'], alpha = 0.3, label = ticker)
    
    if len(windows) > 1:
        ax.plot(data[f'{window[0]} day MA'], alpha = 0.6, label = f'{window[0]} day MA')
        ax.plot(data[f'{window[1]} day MA'], alpha = 0.6, label = f'{window[1]} day MA')
    else:
        ax.plot(data[f'{window[0]} day MA'], alpha = 0.6, label = f'{window[0]} day MA')
        
    ax.scatter(data.index, data.buyPrice, marker = '^', s = 200, color = 'darkblue', label = 'BUY SIGNAL')
    ax.scatter(data.index, data.sellPrice, marker = 'v', s = 200, color = 'crimson', label = 'SELL SIGNAL')
    fig.legend(loc = 'upper left')

#     ax.title(f'{ticker} {strategy} Trading Signals')
...
graph = plot(sma_trade, window, strategy)
st.pyplot(graph)
